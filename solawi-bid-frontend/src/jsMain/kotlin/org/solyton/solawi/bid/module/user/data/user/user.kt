// This file has been partially auto generated. 
// Please don't make any changes to the lenses.
// Feel free to add or remove annotated properties from
// the generator data class. The corresponding lenses 
// will be removed or added on the next run of the 
// lens generator. See below for more details.
package org.solyton.solawi.bid.module.user.data.user

import org.evoleq.optics.Lensify
import org.evoleq.optics.ReadOnly
import org.evoleq.optics.ReadWrite
import org.evoleq.optics.lens.Lens
import org.solyton.solawi.bid.module.permissions.data.Permissions
import org.solyton.solawi.bid.module.user.data.organization.Organization

/**
 * Generator class.
 * Feel free to add or remove annotated properties from
 * the class. Make sure that they are annotated with
 * - @ReadOnly
 * - @ReadWrite
 * If you want that a property-lens will be generated
 * on the next run of the lens generator.
 * If not, just omit the annotation or annotate it with @Ignore.
 */
@Lensify data class User(
    @ReadWrite val username: String = "",
    @ReadWrite val password: String = "",
    @ReadWrite val accessToken: String = "",
    @ReadWrite val refreshToken: String = "",
    @ReadOnly val permissions: Permissions = Permissions(),
    @ReadWrite val organizations: List<Organization>
)

/**
 * Autogenerated Lens.
 * Read and manipulate [User.username]
 */
@ReadWrite val username: Lens<User, String> by lazy{ Lens(
    get = {whole -> whole.username},
    set = {part -> {whole -> whole.copy(username = part)}}
) }
/**
 * Autogenerated Setter of a Pseudo Lens
 * Manipulate [User.username]
 */
@ReadWrite fun User.username(set: String.()->String ): User = copy(username = set(username)) 
/**
 * Autogenerated Lens.
 * Read and manipulate [User.password]
 */
@ReadWrite val password: Lens<User, String> by lazy{ Lens(
    get = {whole -> whole.password},
    set = {part -> {whole -> whole.copy(password = part)}}
) }
/**
 * Autogenerated Setter of a Pseudo Lens
 * Manipulate [User.password]
 */
@ReadWrite fun User.password(set: String.()->String ): User = copy(password = set(password)) 
/**
 * Autogenerated Lens.
 * Read and manipulate [User.accessToken]
 */
@ReadWrite val accessToken: Lens<User, String> by lazy{ Lens(
    get = {whole -> whole.accessToken},
    set = {part -> {whole -> whole.copy(accessToken = part)}}
) }
/**
 * Autogenerated Setter of a Pseudo Lens
 * Manipulate [User.accessToken]
 */
@ReadWrite fun User.accessToken(set: String.()->String ): User = copy(accessToken = set(accessToken)) 
/**
 * Autogenerated Lens.
 * Read and manipulate [User.refreshToken]
 */
@ReadWrite val refreshToken: Lens<User, String> by lazy{ Lens(
    get = {whole -> whole.refreshToken},
    set = {part -> {whole -> whole.copy(refreshToken = part)}}
) }
/**
 * Autogenerated Setter of a Pseudo Lens
 * Manipulate [User.refreshToken]
 */
@ReadWrite fun User.refreshToken(set: String.()->String ): User = copy(refreshToken = set(refreshToken)) 
/**
 * Autogenerated ReadOnly Lens.
 * Read [User.permissions]
 */
@ReadOnly val permissions: Lens<User, Permissions> by lazy{ Lens(
    get = {whole -> whole.permissions},
    set = {{it}}
) }
/**
 * Autogenerated Lens.
 * Read and manipulate [User.organizations]
 */
@ReadWrite val organizations: Lens<User, List<Organization>> by lazy{ Lens(
    get = {whole -> whole.organizations},
    set = {part -> {whole -> whole.copy(organizations = part)}}
) }
/**
 * Autogenerated Setter of a Pseudo Lens
 * Manipulate [User.organizations]
 */
@ReadWrite fun User.organizations(set: List<Organization>.()->List<Organization> ): User = copy(organizations = set(organizations)) 
