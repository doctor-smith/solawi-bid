...
import org.solyton.solawi.bid.module.bid.component.list.NumberCell
...
fun AuctionPage(storage: Storage<Application>, auctionId: String) = Div({style { overflowY("auto" /*todo:dev - scroll only if needed*/)}}) {
    ...
    Vertical({verticalPageStyle(); }) {
        VerticalAccent(verticalAccentStyles(storage * bidApplicationIso * deviceData * mediaType.get)) {
            ...
            Wrap({marginLeft(20.px)}) { H2 { Text((details * title).emit()) } }
            Wrap({marginLeft(20.px)}) {
                Horizontal {
                    AuctionDetails(
        ...
        if((storage * bidApplicationIso * frozenRounds).emit().isNotEmpty()) {
            Wrap {
                ...
                ////////////////////////////////////////////////////////////
                // todo:i18n
                Wrap{H2({/*style { marginLeft(20.px) }*/ }) { Text("Abgeschlossene Runden") }}
                // Header
                Horizontal(styles = {
                ...
                (storage * bidApplicationIso * frozenRounds).emit().forEach { round ->
                    ...
                    Horizontal(styles = {
                        ...
                        Horizontal({ justifyContent(JustifyContent.FlexEnd) }) {
                            val isExportDisabled =
                                (storage * bidApplicationIso * user.get).emit().isNotGranted(BidRight.BidRound.manage)
                            // todo:i18n
                            FileExportButton(
                                Color.black,
                                Color.transparent,
