...
class FiscalYearsCreateTest {

    @OptIn(ComposeWebExperimentalTestsApi::class)
    @Test fun createFiscalYearsTest() = runComposeTest {
        installSerializers()
        val legalEntityId = "legalEntityId"
        val start = LocalDate(2024, 1, 1)
        val end = LocalDate(2024, 12, 31)

        val action = createFiscalYear(
            legalEntityId,
            start,
            end
        )
        ...
        composition {
            ...
            val expectedArgs = CreateFiscalYear(legalEntityId, start, end)
